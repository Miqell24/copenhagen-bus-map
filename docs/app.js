...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: Rejseplanen GTFS — Movia · Metroselskabet · DSB S-tog' }));
  ...
  const PALETTE = [
    ...
    // ships none; keep in sync with CPH_COLORS in pipeline/build.mjs)
    ['#007756', '#00412f'], // M1
    ['#FFCC00', '#8c7000'], // M2
    ['#E40037', '#7d001e'], // M3
    ['#009CD3', '#005674'], // M4
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: Rejseplanen (Movia, Metroselskabet, DSB S-tog)';
      ...
      a.download = `copenhagen-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `copenhagen-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the S-tog reaches Hillerød and Køge, so fitting
  // the data bbox would open on farmland with the city as a small core. Open
  // on central Copenhagen instead.
  map.jumpTo({ center: [12.5683, 55.6761], zoom: 11.6 });
}
