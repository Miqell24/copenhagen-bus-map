...
// Copenhagen: the NATIONAL Rejseplanen feed covers all of Denmark, so the
// map's scope is a PRECOMPUTED allowlist (data/scope.json, pipeline/scope.mjs):
// Movia buses of the city + omegn (3, and the 150S…600S S-buses filed as 700),
// the Hovedstadens Letbane (0), the metro M1–M4 (1) and all seven DSB S-tog
...
const CPH_COLORS = {
  M1: '#007756', M2: '#FFCC00', M3: '#E40037', M4: '#009CD3',
  A: '#00A7E3', B: '#55AB26', Bx: '#AECD6B', C: '#F29400',
  E: '#857AB3', F: '#FCC300', H: '#E64215',
};
...
// ONE feed, FOUR cfgs: routeTypes splits the BKK bundle per mode (3 = bus,
...
const MODES = [{
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['5C']),
  feeds: [
    { tag: 'dk', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['3', '700'],
      skipRoute: (r) => !SCOPE_BUS.has(r.route_id) },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  // Hovedstadens Letbane along Ring 3 — one line, family tram red
  mode: 'tram', label: 'letbane', osmFile: 'data/osm/copenhagen-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'dk', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['0'],
      skipRoute: (r) => !SCOPE_TRAM.has(r.route_id) },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  mode: 'tram', label: 'metro', osmFile: 'data/osm/copenhagen-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'dk', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['1'],
      skipRoute: (r) => !SCOPE_METRO.has(r.route_id), lineColors: CPH_COLORS },
  ],
});
if (tramAll || stogSel.length) MODES.push({
  // all seven S-tog lines, official colors, drawn with the metro treatment
  // (allMetro: the keys are bare letters, so no M-prefix to key off) — on the
  // mainline rail slice of the graph, yards and sidings excluded
  mode: 'tram', label: 'S-tog', osmFile: 'data/osm/copenhagen-rail.json',
  graphMode: 'tram', railKeep: new Set(['rail']),
  railExtra: (e) => e.tags?.railway === 'light_rail',
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : stogSel,
  feeds: [
    { tag: 'dk', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['109'],
...
async function processMode(cfg) {
  ...
  // allMetro: every line of this cfg gets the metro treatment regardless of
  // its name (S-tog keys are bare letters A…H)
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      if (r.route_type === '11') {
        ...
        // official line colors straight from the feed, where a feed ships them
        cfg.lineColors[key] = '#' + r.route_color.toUpperCase();
        cfg.lineColorsDark[key] = darken('#' + r.route_color, 0.45);
      }
      // hand-listed colors for feeds that ship none (Rejseplanen): CPH_COLORS
      if (feed.lineColors && feed.lineColors[key]) {
        cfg.lineColors[key] = feed.lineColors[key];
        cfg.lineColorsDark[key] = darken(feed.lineColors[key], 0.45);
      }
    ...
    // direction_id alone can lie (the Athens/OSY defect, and 22 BKK lines): a
